...
function addItemToLocalStorage(name) {
    try {
        const data = {
            articleHeader: name.parentNode.nextElementSibling.firstElementChild.textContent,
            ...
            newsCategory: name.parentNode.firstElementChild.nextElementSibling.textContent,
        };

        for (let i = 0; i < favoriteNewsData.length; i++) {
            if (favoriteNewsData[i].linkReadMore === name.parentNode.nextElementSibling.lastElementChild.lastElementChild.href) {
                return;
            }
        }
